...
// 바탕화면에 이미지폴더 / 동물이름.txt파일 만들어준 후
	public void method1() {
		String 폴더경로 = "c:/Users/user1/Desktop/이미지";
		...
		if(폴더.exists()) {
			...
			폴더.mkdir();
			System.out.println(폴더.getName() + "폴더가 생성 되었습니다.");
		}

		
		File 텍스트 = new File(폴더 + "/동물이름.txt");
		...
		if(텍스트.exists()) {
			...
			try {
				텍스트.createNewFile();
				System.out.println(텍스트.getName() + "파일이 생성되었습니다.");
			} catch (IOException e) {
				System.out.println("파일 생성에 실패했습니다.");
				e.printStackTrace();
			}
		}
		// 소 고양이 돼지 토끼 호랑이 동물이름을 만들어 준 다음에
		try {
		...
		File 이름변경 = new File(바탕화면경로 + "/이미지/호랑이.txt");
		
		if(텍스트.exists()) {
			if(텍스트.renameTo(이름변경)) {
				System.out.println("파일 이름이 성공적으로 변경되었습니다.");
			} else {
				System.out.println("파일 이름 변경에 실패했습니다.");
			}
		} else {
			System.out.println("파일 이름이 존재하지 않습니다.");
		}
	}
	
	
	// 좋아하는 동물이름을 폴더 이름 변경한 다음
	// 삭제하기
	public static void main(String[] args) {
		...
		try {
			...
			// 삭제하기
			// 폴더 삭제 파일 삭제
			// 폴더 삭제를 할 때 폴더 안에 파일이 존재한다면 폴더 삭제가 되지않음
