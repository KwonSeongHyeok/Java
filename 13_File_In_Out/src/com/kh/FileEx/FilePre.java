...
public class FilePre {
	...
	public void 메서드2() {
		...
		if(폴더.exists()) {
			...
			폴더.mkdir();
			System.out.println(폴더.getName() + "폴더가 생성되었습니다.");
		}

		File 텍스트 = new File(폴더 + "/파일생성.txt");
		
		if(텍스트.exists()) {
			System.out.println("텍스트가 이미 존재합니다.");
		} else {
			try {
				텍스트.createNewFile();
				System.out.println(텍스트.getName() + "텍스트가 생성되었습니다.");
				
			} catch (IOException e) {
				System.out.println("텍스트 생성이 실패했습니다.");
				e.printStackTrace();
			}
